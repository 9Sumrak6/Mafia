...
std::map<int, std::string> role_names = {{0, "Mafia"}, {1, "Doctor"}, {2, "Comissar"}, {3, "Maniac"}, {4, "Civillian"}, {5, "Bull"}, {6, "Executioner"}, {7, "Ninja"}};
...
{
    // {v->vote(*names, msg, mtx, ofs)} -> std::convertible_to<void>;
    // {v->act(*names, msg, mtx, ofs)} -> std::convertible_to<void>;
...
public:
    ...
    virtual void hello(int role, int id) {
        switch(role) {
            case MAFIA:
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is Mafia, kill them all! Good luck!\n\n";
                break;
            case BULL:
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is BULL, kill them all! Maniac can't kill you! Good luck!\n\n";
                break;
            case DOCTOR:
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is Doctor, hill civillians and don't hill Mafia and Maniac! Good luck\n\n";
                break;
            case COMISSAR:
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is Comissar, search and kill Mafia! Good luck\n\n";
                break;
            case MANIAC:
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is Maniac, kill them all and stay 1 in 1 to win! Good luck\n\n";
                break; 
            case CIVILLIAN: 
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is Civillian, find Mafia and Maniac and kick them! Good luck!\n\n" << std::endl;
                break;
            case EXECUTIONER: 
                std::osyncstream(std::cout) << "Your id= " << id << ". Your role is Executioner, find Mafia and Maniac and kick them! Good luck!\n\n" << std::endl;
                break;
        }
    }
    virtual bool update(int &role, std::set<int> &names, std::queue<Message> &q, std::mutex &mtx) {
        ...
        for (auto &i : killed.killed) {
            // std::osyncstream(std::cout) << "chosen player --- " << i << "role= " << role << std::endl;
            if (i == role)
                ans = true;
    ...
    virtual void vote(int &kicked, int role, bool player, std::set<int> &names, std::queue<Message> &msg, std::mutex &mtx, NewPtr<std::barrier<void (*)()>> &bar) {
        ...
        if (player) {
            std::string pls = "Available players to kick: [";

            for (auto &i : names)
                pls += std::to_string(i) + ", ";
            pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';
            do {
                std::osyncstream(std::cout) << pls << "Choose a player from the list above u wanna kick: ";
                std::cin >> choice;
        ...
        if (role == EXECUTIONER) {
            ...
            if (to_kick.size() > 1) {                
                if (player) {
                    to_kick.insert(-1);
                    std::string pls = "Players with the same number of votes. Choose one to kick (-1 to save all): [";

                    for (auto &i : to_kick)
                        pls += std::to_string(i) + ", ";
                    pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';
                    do {
                        std::osyncstream(std::cout) << pls << "Choose a player from the list above u wanna kick (-1 to save all): ";
                        std::cin >> choice;
    ...
    virtual void act(int role, bool player, std::set<int> &names, std::queue<Message> &msg, std::mutex &mtx) {
        ...
        if (player && role != CIVILLIAN && role != EXECUTIONER) {
            std::string pls = "Available players to choose: [";

            for (auto &i : names)
                pls += std::to_string(i) + ", ";
            pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';
            std::osyncstream(std::cout) << pls << "Choose a player from the list above: ";
            std::cin >> choice;
        } else {
            int n = std::rand() % names.size();
            auto it = std::begin(names);
...
// аналогично можно и Executiner и Ninja
...
void Master::kill(NewPtr<std::set<int>> &names, int &maf_num, std::map<int, std::queue<Message>> &q, std::map<int, std::mutex> &mtx, std::ofstream &ofs) {
    ...
    if (maf_votes.size() == 0)
        got_killed = "This night nobody was killed! Continue the game! ";
    else if (maf_votes.size() == 1) 
        got_killed = "This night was killed: ";
    else 
        got_killed = "This night were killed: ";
    ...
    std::cout << got_killed;
    if (maf_act >= 0)
        got_killed += "Mafia killed: " + std::to_string(maf_act) + "; ";
    if (man_act >= 0)
        got_killed += "Maniac killed: " + std::to_string(man_act) + "; ";
    if (com_act >= 0)
        got_killed += "Comissar killed: " + std::to_string(com_act) + "; ";
...
void Master::kick(std::set<int> &names, int &maf_num, std::map<int, std::queue<Message>> &q, std::map<int, std::mutex> &mtx, std::ofstream &ofs) {
    ...
    if (imx >= 0) {
        std::osyncstream(std::cout) << "This day was kicked:  " << imx << std::endl;
        ofs << "This day was kicked:  " << imx << std::endl;
        names.erase(imx);
    } else {
        std::osyncstream(std::cout) << "This day was kicked nobody!\n";
        ofs << "This day was kicked nobody!\n";
    }
...
void Master::act(std::set<int> &names, std::map<int, std::queue<Message>> &q, std::map<int, std::mutex> &mtx, std::ofstream &ofs) {
    ...
    for (auto &i : maf_votes) {
        if (i == doctor_act) {
            ...
            ofs << "This night Doctor hilled: " << i << std::endl;
...
void Master::play(NewPtr<int> &num,
    ...
    if (player)
        std::osyncstream(std::cout) << "You are a spectator, enjoy the game process!\n\n";
    ...
    while (true) {
        ...
        if (finish(num, maf_num, *names, msg, mtx, is_end((*names).size(), maf_num), ofs)) {
            std::string active = "Left players' id: [";
            for (auto &i : *names)
...
bool Master::finish(NewPtr<int> &num, int &maf_num, std::set<int> &names, std::map<int, std::queue<Message>> &q, std::map<int, std::mutex> &mtx, bool end, std::ofstream &ofs) {
    ...
    if (end) {
        switch (m.kill) {
            case 0:
                std::osyncstream(std::cout) << "Civillians win!\n";
                ofs << "Civillians";
                break;
            case 1:
                ...
                break;
            case 2:
                std::osyncstream(std::cout) << "Maniac win!\n";
                ofs << "Maniac";
        }
...
void Mafia::meet(int role, int &maf_num, bool player) {
    ...
    if (player) {
        if (maf_num > 1)
            ...
            std::cout << "You are the only one Mafia!\n";
    }
}
...
void Mafia::act(int role, int &maf_num, bool player, NewPtr<int> &num, std::queue<Message> &msg, std::mutex &mtx) {
    ...
    if (player) {
        std::string pls = "Available players to kill: [";
        for (auto &i : victims)
            pls += std::to_string(i) + ", ";
        pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';
...
void Mafia::play(NewPtr<int> &num,
    ...
    while (true) {
        ...
        if (killed) {
            if (player) std::osyncstream(std::cout) << "You were killed! :(\n" << std::endl;
            break;
        } else if (kicked == role) {
            if (player) std::osyncstream(std::cout) << "You were kicked! :(\n" << std::endl;
            break;
        } else 
...
void Doctor::act(int role, bool player, std::set<int> &names, std::queue<Message> &msg, std::mutex &mtx) {
    ...
    if (player) {
        std::string pls = "Available players to hill: [";
        for (auto &i : names)
            pls += std::to_string(i) + ", ";
        pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';
...
void Comissar::act(int role, bool player, std::set<int> &names, std::queue<Message> &msg, std::mutex &mtx) {
    ...
    if (player) {
        std::string pls = "Available players to check: [";
        for (auto &i : unchecked)
            pls += std::to_string(i) + ", ";
        pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';

        if (mafs.size() > 0) {
            pls += "Available mafias to kill: [";
            for (auto &i : mafs)
                pls += std::to_string(i) + ", ";
            pls[pls.length() - 2] = ']', pls[pls.length() - 1] = '\n';
        }

        std::osyncstream(std::cout) << pls << "Choose a player from the list above and action[0 - check; 1 - kill]: ";
        std::cin >> act >> choice;
    } else if (mafs.size() > 0) {
        choice = *mafs.begin();
        act = 1;
    ...
    if (!act) {
        ...
        if (m.msg != MAFIA && m.msg != BULL){
            if (player) std::osyncstream(std::cout) << "Player with id=" << choice << " is Civillian!\n";
        }
        else {
            if (player) std::osyncstream(std::cout) << "Player with id=" << choice << " is Mafia! Kill him or kick!\n";
            mafs.insert(choice);
        }
    }
}
