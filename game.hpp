...
Game::Game(int n, int k, bool master) {
    ...
    if (extra > 0) {
        do {
            std::cout << "Do u wanna add extra roles? [y\\n]\n>";
            std::cin >> choice;
    ...
    // players_roles.insert(players_roles.end(), this->n - maf_n - 3, CIVILLIAN);
    // players.insert(players.end(), this->n - maf_n - 3, static_cast<Player *>(new Civillian()));
...
void Game::play() {
    ...
    std::cout << "Get ready! The game starts in... 3";
    std::cout.flush();
    sleep(1);
    std::cout << " 2";
    std::cout.flush();
